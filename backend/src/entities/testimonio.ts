import { Entity, Column, PrimaryGeneratedColumn, ManyToOne, JoinColumn, CreateDateColumn, UpdateDateColumn } from 'typeorm';
import { Usuario } from './usuario';
import { Acreedor } from './acreedores';

@Entity({ name: 'testimonios' })
export class Testimonio {
    @PrimaryGeneratedColumn('uuid')
    id!: string;

    @ManyToOne(() => Usuario, { onDelete: 'SET NULL' })
    @JoinColumn({ name: 'id_usuario' })
    usuario!: Usuario | null;

    @ManyToOne(() => Acreedor, { onDelete: 'CASCADE' })
    @JoinColumn({ name: 'id_acreedor' })
    acreedor!: Acreedor;

    @Column({ name: 'monto_original', type: 'decimal', precision: 12, scale: 2 })
    montoOriginal!: number;

    @Column({ name: 'meses_atrasados', type: 'int' })
    mesesTardados!: number;

    @Column({ name: 'estrategia_usada', type: 'varchar', length: 50 })
    estrategiaUsada!: string; // 'snowball', 'avalanche', 'negotiation', 'other'

    @Column({ name: 'mensaje', type: 'text' })
    mensaje!: string;

    @Column({ name: 'consejo', type: 'text', nullable: true })
    consejo!: string | null;

    @Column({ name: 'anonimo', default: true })
    anonimo!: boolean;

    @Column({ name: 'nombre_mostrar', type: 'varchar', length: 100, nullable: true })
    nombreMostrar!: string | null;

    @Column({ name: 'votos_utiles', type: 'int', default: 0 })
    votosUtiles!: number;

    @Column({ name: 'aprobado', default: false })
    aprobado!: boolean;

    @CreateDateColumn({ name: 'creado_en' })
    creadoEn!: Date;

    @UpdateDateColumn({ name: 'actualizado_en' })
    actualizadoEn!: Date;
}